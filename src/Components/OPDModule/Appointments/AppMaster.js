...
const AppMaster = () => {
  ...
  const disableToDate = (currentDate) => {
    ...
    return (
      ...
      (currentDate.isBefore(moment().startOf('minute')) ||
        ...
        (currentDate.isSame(moment(fromDateVal).startOf('day')) &&
          currentDate.isSame(moment(fromDateVal), 'minute')))
    );
  };
  ...
  const handleToDateChange = (date) => {
    ...
    if (
      fromDateValue &&
      date.isSame(fromDateValue, 'day') &&
      date.isSame(fromDateValue, 'minute')
    ) {
      message.warning(
        'To Date cannot be the same time as From Date on the same day.'
      ); // Notify user
      form.setFieldsValue({toDate: null}); // Reset To Date
      setToDate(null); // Reset internal state as well
      return;
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        const response = await OPModuleAgent.getDepartmentMaster();
        if (response.status === 'success' && response.data) {
  ...
  const fetchDoctors = async (departmentId, savedDoctorName = null) => {
    ...
    setLoading(true);
    try {
      const response = await OPModuleAgent.getDoctorListByDepartment(
        departmentId
      );
      if (response.status === 'success' && response.data) {
  ...
  const parseSlotDate = (slotDateString) => {
    ...
    const formattedDateString = `${year}-${month}-${day}T${String(
      hours
  ...
  const filteredTableData1 = tableData1.filter((item) => {
    ...
    return (
      (!fromDateInput || slotDate >= fromDateInput) &&
  ...
  const handleOverallSelect = (e) => {
    ...
    const allRowKeys = new Set(
      isChecked
        ? filteredTableData1.map(
            (item) => `${item.seqnbr}-${item.slotDate}-${item.departmentName}`
          )
        : []
    );
    ...
    },
    {
      title: (
        <Checkbox
          ...
          checked={
            selectedRows.length === filteredTableData1.length &&
            filteredTableData1.length > 0
          }
          indeterminate={
  ...
  const getBlockStatus = () => {
    ...
    const selectedSlots = selectedRows
      .map((seq) =>
        tableData1.find(
          (record) =>
            `${record.seqnbr}-${record.slotDate}-${record.departmentName}` ===
            seq
        )
      )
      .filter((slot) => slot !== undefined);

    const allBlocked = selectedSlots.every(
      (slot) => slot.blockedStatus === 'Blocked'
    );
    const allUnblocked = selectedSlots.every(
      (slot) => slot.blockedStatus === 'Unblocked'
    );
  ...
  const handleBlockUnblock = async () => {
    ...
    if (useManualEntry && seqFrom && seqTo) {
      selectedSlots = tableData1.filter(
        (record) =>
    ...
    if (selectedSlots.length === 0) {
      message.error(
        'Please select at least one slot or enter SeqFrom and SeqTo.'
      );
      return;
    }

    // Determine if blocking or unblocking
    const isBlocking = selectedSlots.some(
      (record) => record.blockedStatus === 'Unblocked'
    );
    const isUnblocking = selectedSlots.every(
      (record) => record.blockedStatus === 'Blocked'
    );
    ...
    if (!isBlocking && !isUnblocking) {
      ...
      message.error(
        'Error: Slot status is unclear. Please check slot details.'
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        FromDate: firstSlot?.slotDate
          ? moment(firstSlot.slotDate, 'DD-MM-YYYY hh:mm A').format(
              'YYYY-MM-DD HH:mm:ss'
            )
          : null,
        ToDate: lastSlot?.slotDate
          ? moment(lastSlot.slotDate, 'DD-MM-YYYY hh:mm A').format(
              'YYYY-MM-DD HH:mm:ss'
            )
          : null,
        CreateUser: userName,
      ...
      if (response?.data?.msgDesc?.toLowerCase().includes('successfully')) {
        message.success(
          `Slots from ${seqFrom || selectedSlots[0]?.seqnbr} to ${
            seqTo || selectedSlots[selectedSlots.length - 1]?.seqnbr
          } ${isBlocking ? 'blocked' : 'unblocked'} successfully.`
        );
        fetchTableData();
        setSelectedRows([]);
        ...
        message.error(
          `Failed to update slots: ${JSON.stringify(response?.data)}`
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (departmentName) {
      const selectedDepartment = departments.find(
        (dep) => dep.mdpt_DepartmentCode_cd === departmentName
      );
      if (selectedDepartment) {
        fetchDoctors(selectedDepartment.mdpt_DepartmentCode_cd);
      }
    }
  ...
  const handleDepartmentChange = (value) => {
    const selectedDepartment = departments.find(
      (dep) => dep.mdpt_DepartmentCode_cd === value
    );
    if (selectedDepartment) {
      setDepartmentId(selectedDepartment.mdpt_DepartmentCode_cd); // Ensure you store department ID as well
      setDepartmentName(value);
      setDoctorName(null);
      form.resetFields(['doctor']);
      fetchDoctors(selectedDepartment.mdpt_DepartmentCode_cd); // Pass departmentId to fetch doctors
  ...
  useEffect(() => {
    const fetchSlotGap = async () => {
      if (selectedDoctorId) {
        try {
          const response = await OPModuleAgent.GetDoctorseqSlotGap(
            selectedDoctorId
          );
  ...
  const handleModify = async (record) => {
    ...
    const selectedDepartment = departments.find(
      (dep) => dep.mdpt_DepartmentName_nm === record.departmentName
    );
    ...
    if (selectedDepartment) {
      await fetchDoctors(
        selectedDepartment.mdpt_DepartmentCode_cd,
        record.doctorName
      ); // Fetch doctors and set doctor in form
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const deletePayload = {
        ...
        ToDatetime: toDate
          ? toDate.format('YYYY-MM-DD HH:mm A')
          : selectedRecord.toDatetime,
      };

      // Calling the delete function
      const response = await OPModuleAgent.deleteDetailslotDatewise(
        deletePayload
      );
      // Checking the response
      if (
        response.status === 'success' &&
        response.data.msgDesc.includes('Deleted Successfully')
      ) {
        ...
        message.error(
          `Failed to delete record: ${JSON.stringify(response.data)}`
        );
      }
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    const confirmed = window.confirm(
      'Are you sure you want to update/add this slot?'
    );
    if (!confirmed) return;
    ...
    if (allowSequenceEdit) {
      // Ignore sequence validation if switch is OFF
      ...
      if (totalSequenceCount > maxCapping) {
        message.error(
          `Total sequence count (${totalSequenceCount}) exceeds the max capping (${maxCapping}).`
        );
        return;
      }
  ...
  const handleSubmit = async (values) => {
    if (!selectedDoctorId || !fromDate || !toDate) {
      message.error(
        'Please make sure to select a doctor and valid date range.'
      );
      return;
    }
    ...
    if (sequenceCount > maxCapping) {
      message.error(
        `Sequence count cannot exceed the Max Capping of ${maxCapping}.`
      );
      return;
    }
    ...
    try {
      ...
      const selectedDepartment = departments.find(
        (dept) => dept.mdpt_DepartmentCode_cd === departmentId
      );
      const selectedDoctor = doctors.find(
        (doc) => doc.columnCode === selectedDoctorId
      );
      ...
      const headerPayload = {
        Slot_Session_Id: 0,
        Doc_DepartmentId: selectedDepartment.mdpt_DepartmentCode_cd,
        DepartmentName: selectedDepartment.mdpt_DepartmentCode_cd,
      ...
      const headerResponse = await OPModuleAgent.SaveMasterHeader(
        headerPayload
      );
      ...
      if (
        !headerResponse ||
        !headerResponse.data ||
        !headerResponse.data.slot_Session_Id
      ) {
        console.error('Header response not valid:', headerResponse);
        message.error(
          'Failed to create appointment header. Please check the input.'
        );
        return;
      }
      ...
      for (
        ...
        const fromTime = currentDate
          .clone()
          .set({hour: fromDate.hour(), minute: fromDate.minute()});
        const toTime = currentDate
          .clone()
          .set({hour: toDate.hour(), minute: toDate.minute()});

        const totalDurationMinutes = toTime.diff(fromTime, 'minutes');
        const slotDurationMinutes = Math.floor(
          totalDurationMinutes / sequenceCount
        );

        for (let slotIndex = 0; slotIndex < sequenceCount; slotIndex++) {
          const slotTime = fromTime
            .clone()
            .add(slotIndex * slotDurationMinutes, 'minutes');
      ...
      const masterDetailPayload = {
        SessionId: sessionId,
        DepartmentId: selectedDepartment.mdpt_DepartmentCode_cd,
        DepartmentName: selectedDepartment.mdpt_DepartmentCode_cd,
      ...
      const detailResponse = await OPModuleAgent.SaveAppointmentMaster(
        masterDetailPayload
      );
  ...
  return (
    <div>
        ...
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Row gutter={16}>
                  ...
                  <Form.Item
                    ...
                    <Select
                      ...
                      {departments.map((department) => (
                        <Option
                          key={department.mdpt_DepartmentCode_cd}
                          value={department.mdpt_DepartmentCode_cd}>
                          {department.mdpt_DepartmentName_nm}
                        </Option>
                      ))}
                    </Select>
                  </Form.Item>
                  ...
                  <Form.Item
                    ...
                    <Spin spinning={loading} tip="Loading doctors...">
                      <Select
                        ...
                        {doctors.map((doctor) => (
                          <Option
                            key={doctor.columnCode}
                            value={doctor.columnCode}>
                            {doctor.columnName}
                          </Option>
                        ))}
                      </Select>
                ...
                <Col span={6}>
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: 'Please select From Date & Time',
                      },
                    ]}>
                    <DatePicker
                ...
                <Col span={6}>
                  <Form.Item
                    ...
                    label="To Date"
                    rules={[
                      {required: true, message: 'Please select To Date & Time'},
                    ]}>
                    <DatePicker
                      showTime={{use12Hours: true, format: 'hh:mm A'}}
                      format="DD-MM-YYYY hh:mm A"
              ...
              <Row gutter={16}>
                <Switch
                  checked={allowSequenceEdit}
                  onChange={(checked) => setAllowSequenceEdit(checked)}
                  checkedChildren="Update Time Only"
                  unCheckedChildren="Enable Addition"
                />
                ...
                <Col span={6}>
                  <Form.Item
                    ...
                    <Input
                      type="number"
                      placeholder="Enter Max Capping"
                      defaultValue={slotGap}
                      readOnly
                    />
                  </Form.Item>
                </Col>
          ...
          <Switch checked={useManualEntry} 
              ...
              <Row gutter={16}>
                <Col span={6}>
                  <Form.Item
                    ...
                    label="Seq From"
                    rules={[
                      {required: useManualEntry, message: 'Enter Seq From'},
                    ]}>
                    <Input
                      type="number"
                      placeholder="Enter Seq From"
                ...
                <Col span={6}>
                  <Form.Item
                    ...
                    label="Seq To"
                    rules={[
                      {required: useManualEntry, message: 'Enter Seq To'},
                    ]}>
                    <Input
                      type="number"
                      placeholder="Enter Seq To"
                      disabled={!useManualEntry}
                    />
                  </Form.Item>
                </Col>
                ...
                <Col span={6}>
                  <Form.Item
                    ...
                    <Input
                      ...
                      defaultValue={form.getFieldValue('reason')}
                    />
                  </Form.Item>
                </Col>
              ...
              <Form.Item style={{textAlign: 'center'}}>
                <Space size="middle">
                  ...
                  <Button
                    ...
                    disabled={
                      !selectedRecord &&
                      !(useManualEntry && manualSelectionValid)
                    }
            ...
            <Col span={12}>
              ...
              <Table
                ...
                columns={columns}
                rowKey={(record) => `session-${record.slot_Session_Id}`}
              />
            </Col>
          </Row>
        </Form>
      ...
      <Table
        dataSource={filteredTableData1}
        columns={columns1}
        rowKey={(record) =>
          `${record.slot_Session_Id}-${record.seqnbr}-${record.slotDate}`
