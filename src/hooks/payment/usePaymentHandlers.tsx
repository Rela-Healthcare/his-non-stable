...
import {pollForPaymentStatus} from '../../utils/PaymentUtil';
import {verifyPaymentStatus} from '../../store/Slices/momentPay/momentPayThunks';
...
export const usePaymentHandlers = ({
  ...
  //           'Please verify payment status manually.'
  //       );
  //       // Initiate status polling
  //       pollForPaymentStatus(id)
  //         .then((status) => {
  ...
  //         })
  //         .catch(() => {
  //           const errorMsg = 'Could not verify payment status';
  //           toast.error(errorMsg);
  //           if (onPaymentError) {
  //             onPaymentError(errorMsg);
  //           }
  ...
  //     }
  //   },
  //   [id, onPaymentSuccess, onPaymentError]
  // );

  const handlePaymentError = useCallback(
    async (error: string, isUserClosed = false) => {
      if (error.includes('404')) {
        toast.info('Verifying payment status...');
        try {
          const status = await dispatch(verifyPaymentStatus(id)).unwrap();
          toast.success('Payment verified successfully');
          if (onPaymentSuccess) {
            onPaymentSuccess(status);
          }
          ...
          const errorMsg =
            'Could not verify payment status automatically. Please check with administrator.';
          toast.error(errorMsg);
          if (onPaymentError) {
            onPaymentError(errorMsg);
          }
    ...
    [dispatch, id, onPaymentSuccess, onPaymentError]
  );
